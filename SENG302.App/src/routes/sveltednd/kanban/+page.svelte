<script lang="ts">
    ...
    //         (item: Task) => item.id === draggedItem.id,
    //     );
...
<div class="h-screen overflow-hidden pt-20 md:pt-0">
    ...
    <div class="h-[calc(100vh-200px)] overflow-x-auto p-8 md:p-16">
        <div
            ...
            style="display: grid; grid-template-columns: 20% 20% 20%;"
        >
            {#each tasksByStatus as { status, items } (status)}
                <div class="flex w-80 flex-none flex-col">
                    ...
                    >
                        <h2 class="text-lg text-swiss-black dark:text-white">
                            {status.replace("-", " ")}
                        </h2>
                        <span
                        ...
                        <div class="space-y-4">
                            {#each items as task, index (task.id)}
                                ...
                                >
                                    ...
                                    >
                                        <h3
                                            class="text-sm text-swiss-black dark:text-white"
                                        >
                                            {task.title}
                                        </h3>
                                        <div
                                            class="h-2 w-2 {getPriorityMarker(
                                                task.priority,
                                            )}"
                                        ></div>
                                    </div>
                                    <p
                                        class="text-xs leading-relaxed text-swiss-mid-gray dark:text-white/60"
                                    >
                                        {task.description}
                                    </p>
                                    <div
                                        class="mt-3 border-t border-swiss-gray pt-3 dark:border-white/10"
                                    >
                                        <span
                                            class="text-xs text-swiss-mid-gray dark:text-white/60"
                                            >{task.priority}</span
